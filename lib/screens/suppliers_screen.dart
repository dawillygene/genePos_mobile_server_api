...
import 'package:gene_pos/services/supplier_service.dart';
...
class _SuppliersScreenState extends State<SuppliersScreen> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Supplier Configuration', style: TextStyle(fontWeight: FontWeight.bold, color: kWhiteColor)),
          ...
          child: FutureBuilder<List<Supplier>>(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                return const Center(child: Text('No suppliers found. Add one to get started!', style: TextStyle(color: kWhiteColor, fontSize: 18)));
              }
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return _buildSupplierListItem(supplier, index);
                },
              );
            },
          ),
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('Add Supplier', style: TextStyle(color: kWhiteColor)),
        tooltip: 'Add a new supplier',
      ),
    );
  }

  Widget _buildSupplierListItem(Supplier supplier, int index) {
                  ...
                  children: [
                    Container(
                      ...
                      child: Column(
                        children: [
                          Icon(
                            supplier == null ? Icons.add_box_rounded : Icons.edit_rounded,
                            size: 40,
                            color: kSecondaryColor,
                          ),
                          const SizedBox(height: 8),
                          Text(
                            supplier == null ? 'Create New Supplier' : 'Update Supplier',
                    ...
                    _buildDialogTextField(nameController, 'Supplier Name', Icons.business),
                    const SizedBox(height: 20),
                    _buildDialogTextField(phoneController, 'Phone (Optional)', Icons.phone),
                     const SizedBox(height: 20),
                    _buildDialogTextField(addressController, 'Address (Optional)', Icons.location_on),
                        ...
                        Expanded(
                          child: ElevatedButton.icon(
                            onPressed: () {
                              ...
                              if (name.isNotEmpty) {
                                if (supplier == null) {
                                  _addSupplier(Supplier(name: name, phone: phone, address: address));
                                } else {
                                  _updateSupplier(supplier.copyWith(name: name, phone: phone, address: address));
                                }
                                Navigator.pop(context);
                              } else {
                                _showSnackBar('Supplier name is required.', Colors.orange);
                              }
                            },
                            ...
                            icon: Icon(
                              supplier == null ? Icons.add_circle : Icons.check_circle,
                              color: kWhiteColor,
                              size: 20,
                            ),
                            label: Text(
  ...
  void _addSupplier(Supplier supplier) async {
    ...
    _loadSuppliers();
    _showSnackBar('Supplier added successfully!', Colors.green.shade600);
  }

  void _updateSupplier(Supplier supplier) async {
    await _supplierService.updateSupplier(supplier);
    _loadSuppliers();
    _showSnackBar('Supplier updated successfully!', Colors.blue.shade600);
  }

  void _deleteSupplier(int id) async {
    await _supplierService.deleteSupplier(id);
    _loadSuppliers();
    _showSnackBar('Supplier deleted successfully!', Colors.red.shade700);
  }
